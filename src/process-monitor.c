...
int main(void)
{
	printf("%s\n", (ret_check_exists(2))?("TRUE"):("FALSE"));
	
	process_t *process = ret_retrieve(1);
	
	printf("%i, '%s', '%s', %i\n", process->pid, process->cmd, process->cmdline, process->ppid);
	
	ret_free(process);
	
	GArray *processes = ret_walk();
	guint i = 0;
	for(i = 0; i < processes->len; i++)
	{
		printf("PID: %i\n", g_array_index(processes, pid_t, i));
	}
